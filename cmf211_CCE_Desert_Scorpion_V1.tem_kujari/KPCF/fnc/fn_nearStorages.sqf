...

{
    if (_x canAdd "Item_FirstAidKit") then {
        KPCF_near_storage pushBack (typeOf _x);
    };
...
// Fill the list
{
    ...
    _ctrlStorage lbAdd (getText (configFile >> _config >> _x >> "displayName"));
} forEach KPCF_near_storage;

[{call KPCF_fnc_nearStorages;}, [], 10] call CBA_fnc_waitAndExecute;
